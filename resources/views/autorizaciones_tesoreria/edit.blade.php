<x-app-layout>
    @section('breadcrumbs')
    <li class="text-sm">
        <a class="flex items-center text-gray-500" href="{{ route('autoriza-tesoreria.index') }}">
            ...
            {{ __('Autorizacion de Requisiciones Tesoreria') }}
        </a>
    </li>
    <li class="text-sm">
    ...
    <div class="max-w-[100rem] grid grid-cols-1 md:grid-cols-2 gap-4 sm:px-6 lg:px-8 lg:py-2 mx-auto">
        ...
        <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
             <form id="formulario_central" method="post" action="{{ route('autoriza-tesoreria.update', $requisicion) }}">
                @csrf
                ...
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('autoriza-tesoreria.index') }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                        {{ __('Cancel') }}
                        </a>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                 <thead>
                        <tr>
                            ...
                            <th scope="col" colspan="3" class="px-6 py-1 text-center">
                  ...
                  <tbody id="tabla-detalles" class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="h-px w-px whitespace-nowrap">
                            <div class="px-6 py-1 text-right">
                                <span class="text-sm text-gray-600">{{ number_format($detalle->CANT_AUT2 * $detalle->PU, 2, '.', ',') }}</span>
                            </div>
                        </td>
    ...
    <div id="hs-autorizar" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto" role="dialog" aria-labelledby="hs-autorizar-label">
        ...
        <div class="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <form action="{{ route('autoriza-tesoreria.autorizar', $requisicion)}}" method="post">
    ...
    <div id="hs-rechazar" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto" role="dialog" aria-labelledby="hs-rechazar-label">
        ...
        <div class="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <form action="{{ route('autoriza-tesoreria.rechazar', $requisicion)}}" method="post">
                <div class="p-4 sm:p-10 overflow-y-auto">
                    <div class="flex gap-x-4 md:gap-x-7">
                        <!-- Icon -->
    ...
    <script>
        ...
        function obtenerDetalle(crearOEditarDetalle, detalleId = 0)
        {
            ...
            if (crearOEditarDetalle == 1) {
                ...
                fetch("{{ route('detalle.tesoreria.edit', ':detalle') }}".replace(':detalle', detalleId))
                .then(response => response.json())
                .then(function(data) {
                    document.getElementById('unidad_medida').value = data.detalle.UNI_MED;
                    document.getElementById('cantidad').value = data.detalle.CANT_AUT2;
        ...
        function generarDetalle()
        {
            if (clickEnabled) {
              ...
              if (crearOEditar == 1){
                  ...
                  url = "{{ route('detalle.tesoreria.update', ':detalle') }}";
                  url = url.replace(':detalle', detalleSelected);
              }
                ...
                for(var i = 0; i <= data.length -1 ; i++) {
                    document.getElementById("tabla-detalles").insertAdjacentHTML("beforeend", `
                        <tr>
                          ...
                          <td class="h-px w-px whitespace-nowrap">
                              <div class="px-6 py-1 text-right">
                                  <span class="text-sm text-gray-600">${ parseFloat(data[i].PU * data[i].CANT_AUT2).toFixed(2) }</span>
                              </div>
                          </td>
        ...
        async function actualizarTotal() {
            try {
                ...
                const response = await fetch("{{ route('autoriza-tesoreria.obterner_total', $requisicion) }}");
