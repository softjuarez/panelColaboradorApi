...
<head>
    ...
    <style>
        ...
        .replacement-option input[type="checkbox"] {
            ...
            margin-right: 5px;
            margin-left: 0;
            transform: scale(1.5); /* Ajustar el tamaño del checkbox */
        }
...
<body>
    <div class="container">
        ...
        <div class="field-row">
            <b>NOMBRE DEL SOLICITANTE: </b> {{ $licencia->ficha->NOMBRE }}
        </div>

        <div class="field-row">
            <b>NOMBRE JEFE INMEDIATO: </b>{{ $licencia->ficha->jefeInmediato() }}</span>
        </div>

        <div class="field-row">
            <b>SOLICITUD DE PERMISO PARA: </b>
        </div>
        ...
        <div class="replacement-form">
            ...
            <div class="replacement-option">
                <input type="checkbox" id="casamiento" {{ $licencia->CASAMIENTO_SN == 'S' ? 'checked' : '' }}>
                <label for="cuentaVacaciones">Casamiento</label>
            </div>
            <div class="replacement-option">
                <input type="checkbox" id="nacimiento" {{ $licencia->NAC_HIJO == 'S' ? 'checked' : '' }}>
                <label for="descuentoSalario">Nacimiento Hijo</label>
            </div>
            <div class="replacement-option">
                <input type="checkbox" id="fallecimiento" {{ $licencia->FALLECE_SN == 'S' ? 'checked' : '' }}>
                <label for="otros">Fallecimiento (papa, hijos, conyugue)</label>
            </div>
            <div class="replacement-option">
                <input type="checkbox" id="audiencias" {{ $licencia->AUD_O_PROC_SN == 'S' ? 'checked' : '' }}>
                <label for="otros">Audiencias o procesos legales</label>
            </div>
            <div class="replacement-specific">
                <span class="field-label">OTROS ESPECIFIQUE:</span>
                <br/>
                <br/>
                <span>{{ rtrim($licencia->OTROS_ESP) }}</span>
            </div>
        </div>
        ...
        <div class="date-info">
            <span><b>Fecha:</b> {{ $fecha_inicio->isoFormat('[Guatemala, ]D [de] MMMM [de] YYYY'); }}</span>
        </div>
    </div>
</body>
</html>
