<x-app-layout>
  @section('breadcrumbs')
    <li class="text-sm">
        <a class="flex items-center text-sm text-gray-500 hover:text-blue-600 focus:outline-none focus:text-blue-600 dark:focus:text-blue-500" href="{{ route('feriados.index') }}">
    ...
    <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
      <div class="mb-8">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ __('Formulario para editar feriados') }}.
        </p>
      </div>

      <form method="post" action="{{ route('feriados.update', $feriado) }}">
        @csrf
        <!-- Grid -->
        <div class="grid sm:grid-cols-12 gap-2 sm:gap-6">
          ...
          <div class="sm:col-span-3">
            <label for="fecha" class="inline-block text-sm text-gray-800 mt-2.5 dark:text-gray-200">
              {{ __('Fecha') }}
            </label>
          </div>
          <!-- End Col -->
          ...
          <div class="sm:col-span-9">
            <div class="sm:flex">
              <input name="fecha" id="fecha" type="date" class="py-2 px-3 block w-full border-gray-200 -mt-px -ms-px first:rounded-t-lg last:rounded-b-lg sm:first:rounded-s-lg sm:mt-0 sm:first:ms-0 text-sm relative focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600" value="{{ @old('fecha', $feriado->fecha) }}">
            </div>
