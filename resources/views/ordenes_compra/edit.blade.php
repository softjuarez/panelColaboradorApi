...
<x-app-layout>
    @section('breadcrumbs')
    <li class="text-sm">
        <a class="flex items-center text-gray-500" href="{{ route('ordenes_compra.index') }}">
        ...
        <div>
            <div class="block ">
                <nav class="flex space-x-2" aria-label="Tabs" role="tablist" hs-data-tab-select="#tab-select">
                    <button type="button" class="hs-tab-active:bg-white hs-tab-active:dark:bg-slate-900 hs-tab-active:text-gray-600 -mb-px py-3 px-7 inline-flex items-center gap-2 bg-gray-50 dark:bg-slate-900 text-sm font-medium text-center text-gray-500 rounded-t-lg hover:text-gray-700 active [box-shadow:0_0px_3px_rgba(0.2,0.2,0.2,0)]" id="hs-tab-to-select-item-1" data-hs-tab="#hs-tab-to-select-1" aria-controls="hs-tab-to-select-1" role="tab">
                        Detalle
                    </button>

                    <button type="button" class="hs-tab-active:bg-white hs-tab-active:dark:bg-slate-900 hs-tab-active:text-gray-600 -mb-px py-3 px-7 inline-flex items-center gap-2 bg-gray-50 dark:bg-slate-900 text-sm font-medium text-center text-gray-500 rounded-t-lg hover:text-gray-700  box-shadow:0_0px_3px_rgba(0.2,0.2,0.2,0)]" id="hs-tab-to-select-item-2" data-hs-tab="#hs-tab-to-select-2" aria-controls="hs-tab-to-select-2" role="tab">
            ...
            <div>
                    ...
                    <div class="bg-white rounded-xl rounded-t-none shadow p-4 sm:p-7 dark:bg-slate-900">
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                <a href="{{ route('ordenes_compra.index') }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                                {{ __('Regresar') }}
                                </a>

                                @if ($orden->ESTATUS == 'A')
                                <a href="{{ route('ordenes_compra.procesar', $orden) }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-orange-600 text-white hover:bg-orange-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">Procesar</a>
                                @endif

                                <a href="{{ route('ordenes_compra.reporte', $orden) }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" target="_blank">Reporte</a>
                            </div>
                        </div>
                        ...
                        <div class="mt-2">
                            <table class="dark:text-gray-200 text-sm">
                                ...
                                <tr>
                                    <td><b>Total:</b></td>
                                    <td colspan="3" class="px-6">{{ number_format($orden->VLR_FOB, 2, '.', ',') }}</td>
                                </tr>
                                <tr>
                ...
                <div id="hs-tab-to-select-2" class="hidden" role="tabpanel" aria-labelledby="hs-tab-to-select-item-2">
                    <div class="bg-white rounded-xl rounded-t-none shadow p-4 sm:p-7 dark:bg-slate-900">
                        ...
                        <div class="mt-2">
                            @if($orden->documentos->where('estatus', '<>', '1')->isNotEmpty())
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 mt-4">
                                ...
                                <tbody>
                                    @foreach($orden->documentos->where('estatus', '<>', 1) as $documento)
                                    <tr @class([
                                        ...
                                        <td class="px-4  text-center items-center flex justify-end">
                                            <a href="{{ route('documentos.edit', $documento) }}" class="flex items-end justify-center px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800" title="{{ __('Ver documento') }}">
        ...
        <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    {{ __('Datos de Requisicion') }}
                    </h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        {{ __('Datos generales de la requisicion.') }}
                    </p>
                </div>
            </div>
            ...
            <div class="mt-2">
                <table class="dark:text-gray-200 text-sm">
                    <tr>
                        <td><b>Numero:</b></td>
                        <td class="px-6">{{ rtrim($orden->requisicion->NUMERO) }}</td>
                        <td><b>Fecha:</b></td>
                        <td class="px-6">{{ date('d/m/Y', strtotime($orden->requisicion->FECHA)) }}</td>
                    </tr>
                    <tr>
                        <td><b>Tipo Cambio:</b></td>
                        <td class="px-6">{{ number_format($orden->requisicion->TIPO_CAMBIO, 2, '.', '') }}</td>
                    </tr>
                    <tr>
                        <td><b>Solicitante:</b></td>
                        <td colspan="3" class="px-6">{{ Str::of($orden->requisicion->SOLICITANTE)->title() }}</td>
                    </tr>
                    <tr>
                        <td><b>Tipo Compra:</b></td>
                        <td colspan="3" class="px-6">{{ Str::of($orden->requisicion->tipoCompra())->title() }}</td>
                    </tr>
                    <tr class="{{ $orden->requisicion->T_RECHAZO != 'A' ? 'hidden' : '' }}">
                        <td><b>Asignado a:</b></td>
                        <td colspan="3" class="px-6">{{ Str::of($orden->requisicion->usuarioDestino())->title() }}</td>
                    </tr>
                    <tr>
                        <td><b>Lugar de entrega:</b></td>
                        <td colspan="3" class="px-6">{{ !empty($orden->requisicion->COMENTARIO2) ? Str::of($orden->requisicion->COMENTARIO2)->title() : 'Sin especificar' }}</td>
                    </tr>
                    <tr>
                        <td><b>Comentarios:</b></td>
                        <td colspan="3" class="px-6">{{ Str::of($orden->requisicion->COMENTARIOS)->title() }}</td>
                    </tr>
                </table>
            </div>
        </div>
