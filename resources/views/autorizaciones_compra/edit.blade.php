<x-app-layout>
    @section('breadcrumbs')
    <li class="text-sm">
        <a class="flex items-center text-gray-500" href="{{ route('autoriza-compra.index') }}">
    ...
    <div class="max-w-[100rem] grid grid-cols-1 md:grid-cols-2 gap-4 sm:px-6 lg:px-8 lg:py-2 mx-auto">
            ...
            <div class="mb-2">
                <div class="flex items-start justify-between">
                  ...
                  <div class="flex items-center gap-2">
                      @if ($requisicion->ESTATUS == 'E' || $requisicion->ESTATUS == 'G')
        ...
        <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
            <form id="formulario_central" method="post" action="{{ route('autoriza-compra.update', $requisicion) }}">
                @csrf
                ...
                <div class="grid sm:grid-cols-12 gap-2 sm:gap-3 my-2">
                    ...
                    <div class="sm:col-span-3">
                        <label for="gestor" class="inline-block text-sm text-gray-800 mt-2.5 dark:text-gray-200">
                            {{ __('Gestor*') }}
                        </label>
                    </div>

                    <div class="sm:col-span-9">
                        <select name="gestor" id="gestor" class="py-2 px-3 block w-full border-gray-200 -mt-px -ms-px first:rounded-t-lg last:rounded-b-lg sm:first:rounded-s-lg sm:mt-0 sm:first:ms-0  sm:last:rounded-e-lg text-sm relative focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600">
                            <option selected disabled>Seleccionar</option>
                            @foreach ($gestores as $gestor)
                            <option value="{{ $gestor->id }}" @selected(@old('gestor', $requisicion->GESTOR_ID) == $gestor->id)>{{ $gestor->usuario->name }}</option>
                            @endforeach
                        </select>

                        <x-input-error class="mt-2" :messages="$errors->get('gestor')" />
                    </div>
                </div>
            </form>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead>
                    <tr>
                        ...
                        <!-- Encabezado principal para Cantidades -->
                        <th scope="col" colspan="{{ $requisicion->ESTATUS == 'G' ? 5 : 3 }}" class="px-6 py-1 text-center">
                  ...
                  <tbody id="tabla-detalles" class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="h-px w-px whitespace-nowrap">
                            <div class="px-6 py-1 text-right">
                                <span class="text-sm text-gray-600">{{ number_format($detalle->CANT_OC * $detalle->PU, 2, '.', ',') }}</span>
                            </div>
                        </td>
    ...
    <div id="hs-autorizar" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto" role="dialog" aria-labelledby="hs-autorizar-label">
        ...
        <div class="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <form action="{{ route('autoriza-compra.autorizar', $requisicion)}}" method="post">
    ...
    <div id="hs-rechazar" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto" role="dialog" aria-labelledby="hs-rechazar-label">
        ...
        <div class="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <form action="{{ route('autoriza-compra.rechazar', $requisicion)}}" method="post">
                <div class="p-4 sm:p-10 overflow-y-auto">
                    <div class="flex gap-x-4 md:gap-x-7">
                        <!-- Icon -->
    ...
    <script>
        ...
        function obtenerDetalle(crearOEditarDetalle, detalleId = 0)
        {
            ...
            if (crearOEditarDetalle == 1) {
                ...
                calcularTotal();
                new HSSelect(document.querySelector('#centro_costo'));
            } else {
                ...
                .then(function(data) {
                    ...
                    document.getElementById('cantidad').value = data.detalle.CANT_OC;
        ...
        function generarDetalleP()
        {
            if (clickEnabled) {
              ...
              const datos = JSON.stringify({
                concepto: detalleP.CONCPPTO, //document.getElementById('concepto').value,
        ...
        function generarDetalle()
        {
            if (clickEnabled) {
              ...
              if (crearOEditar == 1){
                  ...
                  url = "{{ route('detalle.compra.update', ':detalle') }}";
                  url = url.replace(':detalle', detalleSelected);
              }
                ...
                for(var i = 0; i <= data.length -1 ; i++) {
                    document.getElementById("tabla-detalles").insertAdjacentHTML("beforeend", `
                        ...
                        @endif
                          ...
                          <td class="h-px w-px whitespace-nowrap">
                              <div class="px-6 py-1 text-right">
                                  <span class="text-sm text-gray-600">${ parseFloat(data[i].PU * data[i].CANT_OC).toFixed(2) }</span>
                              </div>
                          </td>
        ...
        async function actualizarTotal() {
            try {
                const response = await fetch("{{ route('autoriza-compra.obterner_total', $requisicion) }}");
    ...
    <script>
        ...
        {
            andleSubmitButtonLoading('cargar_archivo');
            ...
            .then(function(responseData) {
                ...
                handleSubmitButtonStopLoading('cargar_archivo');
            });
        }
