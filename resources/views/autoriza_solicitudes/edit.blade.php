<x-app-layout>
    @section('breadcrumbs')
      <li class="text-sm">
          <a class="flex items-center text-sm text-gray-500 hover:text-blue-600 focus:outline-none focus:text-blue-600 dark:focus:text-blue-500" href="{{ route('autoriza_solicitudes.index') }}">
        ...
        <div>
            <div id="card-type-tab-preview" role="tabpanel" aria-labelledby="card-type-tab-item-1">
                <div class="bg-white rounded-xl rounded-t-none shadow p-4 sm:p-7 dark:bg-slate-900">
                        ...
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                    {{ __('Formulario para editar solicitudes') }}.
                                </p>
                            </div>
                    ...
                    <form method="post" action="{{ route('autoriza_solicitudes.update', $solicitud) }}">
                        @csrf
                        ...
                        <div class="mt-5 flex justify-end gap-x-2">
                            <a href="{{ route('autoriza_solicitudes.index') }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                            {{ __('Cancel') }}
                            </a>
                            @if ($solicitud->estatus == 'B')
    ...
    <div id="hs-autorizar" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto" role="dialog" aria-labelledby="hs-autorizar-label">
        ...
        <div class="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <a href="{{ route('autoriza_solicitudes.autorizar', $solicitud) }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-teal-500 text-white hover:bg-teal-600 disabled:opacity-50 disabled:pointer-events-none">
                Autorizar
            </a>
            </div>
        </div>
    ...
    <div id="hs-rechazar" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto" role="dialog" aria-labelledby="hs-rechazar-label">
        ...
        <div class="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <form action="{{ route('autoriza_solicitudes.rechazar', $solicitud)}}" method="post">
                <div class="p-4 sm:p-10 overflow-y-auto">
                    <div class="flex gap-x-4 md:gap-x-7">
                        <!-- Icon -->
    ...
    <script>
        ...
        function cargarPagina(pagina) {
            var url = '{{ route("adjuntos.solicitud.listado", $solicitud) }}';
        ...
        function subirAdjunto()
        {
            ...
            var url = '{{ route("adjuntos.solicitud.store", $solicitud) }}';
        ...
        {
            fetch("{{ route('adjuntos.solicitud.delete', ':file') }}".replace(':file', $file_id))
            .then(response => response.json())
            .then(function(responseData) {
                cargarPagina();
            });
