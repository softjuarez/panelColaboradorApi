<x-app-layout>
  @section('breadcrumbs')
    <li class="text-sm">
        <a class="flex items-center text-sm text-gray-500 hover:text-blue-600 focus:outline-none focus:text-blue-600 dark:focus:text-blue-500" href="{{ route('tipo_permisos.index') }}">
    ...
    <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
      <div class="mb-8">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ __('Formulario para editar tipos de permisos') }}.
        </p>
      </div>

      <form method="post" action="{{ route('tipo_permisos.update', $tipo_permiso) }}">
        @csrf
        <!-- Grid -->
        ...
        <div class="mt-5 flex justify-end gap-x-2">
          <a href="{{ route('tipo_permisos.index') }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
            {{ __('Cancel') }}
          </a>
