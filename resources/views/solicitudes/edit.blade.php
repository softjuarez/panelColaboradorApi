<x-app-layout>
    ...
    <script>
        ...
        function cargarPagina(pagina) {
            var url = '{{ route("archivos.solicitud.listado", $solicitud) }}';
        ...
        function subirAdjunto()
        {
            ...
            var url = '{{ route("archivos.solicitud.store", $solicitud) }}';
        ...
        {
            fetch("{{ route('archivos.solicitud.delete', ':file') }}".replace(':file', $file_id))
            .then(response => response.json())
            .then(function(responseData) {
                cargarPagina();
            });
        ...
        @if ($solicitud->estatus == 'A')
        let clicked = true;
        document.getElementById('solicitarBtn').addEventListener('click', function(e) {
            if (!clicked) return;
            const btnText = document.getElementById('btnText');
            this.disabled = true;
            btnText.innerHTML = `
                <span class="inline-block h-4 w-4 mr-2 animate-spin rounded-full border-2 border-solid border-current border-e-transparent align-[-0.125em] text-surface motion-reduce:animate-[spin_1.5s_linear_infinite] dark:text-white" role="status">
                    <span class="!absolute !-m-px !h-px !w-px !overflow-hidden !whitespace-nowrap !border-0 !p-0 ![clip:rect(0,0,0,0)]">Cargando...</span>
                </span>
                Procesando...
            `;

            window.location.href = "{{ route('solicitudes.cerrar', $solicitud) }}";
            clicked = false;
        });
        @endif
    </script>
