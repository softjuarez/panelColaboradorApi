<x-app-layout>
    @section('breadcrumbs')
      <li class="text-sm">
          <a class="flex items-center text-sm text-gray-500 hover:text-blue-600 focus:outline-none focus:text-blue-600 dark:focus:text-blue-500" href="{{ route('licencias.index') }}">
                ...
                <div class="bg-white rounded-xl rounded-t-none shadow p-4 sm:p-7 dark:bg-slate-900">
                    <div class="mb-2">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                    {{ __('Formulario para editar licencias') }}.
                                </p>
                            </div>

                            <div class="flex items-center gap-2">
                              @if ($licencia->estatus == 'A')
                              <a href="{{ route('licencias.reporte', $licencia) }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-teal-600 text-white hover:bg-teal-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" target="_blank">Licencia</a>

                              <a href="{{ route('licencias.cerrar', $licencia) }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-orange-600 text-white hover:bg-orange-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">Solicitar</a>
                              @endif
                        ...
                        <div class="mt-2">
                            <table class="dark:text-gray-200 text-sm">
                                ...
                                <tr>
                                    <td><b>Jefe:</b></td>
                                    <td class="px-6">{{ $licencia->atendio->name ?? 'Sin atender' }}</td>
                                    <td><b>Fecha Solicitado:</b></td>
                                    <td class="px-6">{{ date('d/m/Y', strtotime($licencia->fecha_evento)) }} - {{ date('d/m/Y', strtotime($licencia->fecha_fin_evento)) }}</td>
                                </tr>
                                <tr>
                                    <td><b>Estado:</b></td>
                    ...
                    <form method="post" action="{{ route('licencias.update', $licencia) }}">
                            ...
                            <div class="sm:col-span-9">
                                <div class="sm:flex">
                                    <select name="tipo" id="tipo" class="py-2 px-3  block w-full border-gray-200 -mt-px -ms-px first:rounded-t-lg last:rounded-b-lg sm:first:rounded-s-lg sm:mt-0 sm:first:ms-0 sm:first:rounded-se-none sm:last:rounded-es-none sm:last:rounded-e-lg text-sm relative focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600">
                                        ...
                                        <option value="{{ $tipo->id }}" @selected($tipo->id == @old('tipo', $licencia->tipo))>{{ $tipo->nombre }}</option>
                                        @endforeach
                                    </select>
                                    <input name="fecha_inicio" id="fecha_inicio" type="date" class="py-2 px-3 block w-full border-gray-200 -mt-px -ms-px first:rounded-t-lg last:rounded-b-lg sm:first:rounded-s-lg sm:mt-0 sm:first:ms-0 sm:first:rounded-se-none sm:last:rounded-es-none sm:last:rounded-e-lg text-sm relative focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600" placeholder="Dias Credito" value="{{ @old('fecha', $licencia->fecha_evento) }}">
                                </div>
                        ...
                        <div class="mt-5 flex justify-end gap-x-2">
                            <a href="{{ route('licencias.index') }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                            {{ __('Cancel') }}
                            </a>
                            @if ($licencia->estatus == 'A')
        ...
        <div class="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            ...
            <div class="flex gap-x-4 md:gap-x-7">
                ...
                <p class="text-gray-500 dark:text-neutral-500">
                    {{ $licencia->RAZON_RECHAZO_SOLICITANTE }}
                </p>
                </div>
            </div>
            </div>
    ...
    <script>
        function subirAdjunto()
        {
            ...
            var url = '{{ route("licencias.adjunto", $licencia) }}';
