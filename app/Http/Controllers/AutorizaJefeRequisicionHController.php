...
class AutorizaJefeRequisicionHController extends Controller
{
    public function index($search = "")
    {
        ...
        $requisiciones = RequisicionH::whereIn('SOLICITANTE_NUM', $fichas->toArray())->where('SOLICITANTE', 'LIKE', '%'.$search.'%')->where('ESTATUS', 'B')->orderBy('NUMERO', 'desc')->paginate(15);
        return view('autorizaciones_jefe.index')->with(['requisiciones' => $requisiciones, 'search' => $search]);
    }
    ...
    public function edit(RequisicionH $requisicion)
    {
        ...
        return view('autorizaciones_jefe.edit')->with(['requisicion' => $requisicion, 'empresas' => $empresas, 'monedas' => $monedas, 'proveedores' => $proveedores, 'centros_costo' => $centros_costo, 'conceptos' => $conceptos, 'renglones' => $renglones, 'cuentas' => $cuentas, 'sedes' => $sedes, 'fichas' => $fichas]);
    }
    ...
    public function update(Request $request, RequisicionH $requisicion)
    {
        ...
        return redirect()->route('autoriza-jefe.edit', $requisicion)->with(['status' => 'success', 'message' => 'Requisicion updated successfully!']);
    }
    ...
    public function autorizar(Request $request, RequisicionH $requisicion)
    {
        ...
        $requisicion->USR_AUT1 = Str::limit(auth()->user()->name, 255, '');
        $requisicion->FECHA_AUT1 = date('Y-m-d');
        $requisicion->HORA_AUT1 = date('H:i');
        $requisicion->save();

        return redirect()->route('autoriza-jefe.index')->with(['status' => 'success', 'message' => 'Requisición autorizada correctamente.']);
    }
    ...
    public function rechazar(Request $request, RequisicionH $requisicion)
    {
        ...
        return redirect()->route('autoriza-jefe.index')->with(['status' => 'success', 'message' => 'Requisición rechazada correctamente.']);
    }
